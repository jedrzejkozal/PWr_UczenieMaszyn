...
All tests were performed with Pytohn 3.6. Keras implementations of CNN and scikit-learn implementation of LDA and PCA were emplyed to perform tests. Some image operation during preprocessing of images were performed with opencv3 library. Svm and MLP implementations were imported from scikit-learn. Statistical analysis was performed using numpy, scipy library stats module, and implementation of Nemenyi post hoc test avaliable in \cite{post_hoc_nemenyi}.
...
CNN architecture was based on 2 convolution layers with 7x7 and 3x3 kernels respectively, 2 pooling layers with 2x2 kernels and dense layer. Convolution layers contained Rectified Linear Unit (ReLU) activation functions. Dense layer used ReLU and Softmax activation functions. Last layer had 100 outputs for 100 components respectively.
...
For both SVC and MLP performance obtained by CNN was better then performance of PCA. No statistical difference was noted between LDA and PCA, although PCA with SVC had some clear problems with some databases, especially with datasets with different face orientation (e.g. att, georgia, muct or stirling).
