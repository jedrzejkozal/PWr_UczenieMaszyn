...
Also important issue with Stirling faces database was found. With aprroximate 9 images per class 10 fold Stratified Cross Validation cannot be performed. $N$ fold Stratified Cross Validation requiers at least $N$ images for each class. To overcome this problem oversampling of Stirling database was introduced.
...
\begin{table}[!h]
    \centering
    \caption{Pvalues of Neymani posthoc tests for Svc classifier.}
    \begin{tabular}{|c|c|c|c|}
         \hline
          & CNN & LDA & PCA \\
         \hline
...
Significant diffrence in performence can be noted between CNN and PCA.
One might find strage, that pvalue for diffrence between CNN and LDA was lower then pvalue for LDA and PCA. This is conected to problem with Nemenyi test mentioned in begining of this section. For some datasets PCA obtained better acuracy then LDA, or even CNN. The diffrence in ranks beetween average ranks for CNN and LDA was 0.791 and for LDA and PCA was 0.542. Bigger diffrence in average ranks means lower probability, that diffrence is lower then standard error, therefore pvalue is lower. 
...
Very low pvalue was reported for LDA and PCA, and this is in line with obtained diffrence in average ranks equal 0.875.


\section{Conclusions}

Experiments were carried out with established methodology. Results were colected and analysed. Some flaws in chosen methodology were pointed out, and considered durning analysis of results.

For both SVC and MLP performance obtained by CNN was better then performance of PCA. No statistical diffrence was noted beetween LDA and PCA, although PCA with SVC had some clear problems with some databases, especially with datasets with diffrent face orientation (eg. att, georgia, muct or stirling).

Time of traninig was reported in this article, but only for illustrative purposes. No statistical analysis of traning time was performed.

%During tests perfming some problems occured. For some dataset with large number of images Python virtual machine run out of memory on machine with 8 Gb RAM. This happend during computing eigenvalues and eigenvectors of $S_W$ matrix (when establishing within variance) while performing LDA for vidtimit dataset. $A_i$ (see equation \ref{eq:SW}) contains only images for $i$-th class. Maximal number of images for one class in vidtimit is 2644. Images are stored using 8 bit uint type. All images are converted to greyscale, so there is one 8 bit unsigned int per pixel. Sckit-learn use SVD implementation from numpy to establish eigenvalues and eigenvectors of $S_W$. In theory memory needed for storage of SVDs decomposition matrixes given all previous asumptions is aproximatly 0.25 Gb, which should not be problem. Numpy utilises Lapack library for SVD computation. Lapack is linear algebra library written in Fortran. There is little known about alorithms used to perform SVD in Lapack, therefore reason of raising MemoryError cannot be established. This problem was omitted by performing tests on machine with 16 Gb RAM. 
